...
# model related
parser.add_argument("--model",default="lenet5",help="The name of neural networks",choices=["resnet18","squeezenet","lenet5",
    ...
    for epoch in range(0,args.epoch,1):
        ...
        for i, data in enumerate(critrainloader, 0):
            ...
            if iteration % 100 == 0:
                ...
                valcost += time.time() - start_val # record the validation cost of each epoch
                local_wallclock = wallclock + time.time() - starteg - valcost
                _headeri="[ {} Epoch {} /Iteration {} Wallclock {}]".format(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())),epoch+1,iteration,
        local_wallclock)
                print('{} Accuracy of the network on the 10000 test images: {} %'.format(_headeri,100 * correct / total))
                logger.write('{},{},{},{}\n'.format(epoch+1 ,iteration, local_wallclock, 100 * correct / total))
        ...
        wallclock += time.time() - starteg - valcost
        # test on test data
        correct = 0
        total = 0
        with torch.no_grad():
