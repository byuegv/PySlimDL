...
def main(args,*k,**kw):
    ...
    # load traing data
    trainset = dataloader.TinyImage(root=args.dataset,class_num=args.classes, train=True, download=False, transform=None)
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.batchsize, shuffle=True, num_workers=0)

    testset = dataloader.TinyImage(root=args.dataset,class_num=args.classes, train=False, download=False, transform=None)
    testloader = torch.utils.data.DataLoader(testset, batch_size=args.batchsize, shuffle=False, num_workers=0)

    # construct neural network
    net = models.AlexNet(args.classes)
    net.to(device)
    ...
    for epoch in range(0,args.epoch,1):
        ...
        if args.noncriticalremove == False:
            ...
            critrainset = dataloader.CriticalTinyImage(datax=ndata,targetsx=nlabels)
        else:
